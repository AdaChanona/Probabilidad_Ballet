...
def combinaciones_bailarinas():
    equipo=combinations(bailarinas,2)
...
def probabilidad_clasica(df_ballet):
    ...
    pasos_salida=['P(Retiré)','P(Relevé)']
    df1=pd.DataFrame(pasos_salida,columns=['Salidas'])
...
def probabilidad_subjetiva_tabla1(df_ballet):
    ...
    frec_pasos_impar=[]
    for i in range(len(posiciones_ballet)):
            contador=posiciones_ballet.count(i+1)
            frac=contador/len(posiciones_ballet)
            frac=Fraction(frac).limit_denominator()
            prob_posiciones.append(frac*contador)
            if pasos[i]=='Relevé':
    ...
    df_subjetiva1.to_csv('./Resultados/Probabilidad_subjetiva_antes.csv')
    return df_subjetiva1,prob_posiciones

def probabilidad_subjetiva_tabla2(prob_pos, df_ballet):
    pasos=np.array(df_ballet['FASE 2 PASOS'])
    pos=np.array(df_ballet['FASE 1 POSICIONES'])
    pasos_fraccion_par=[]
    ...
    for i in range(len(posiciones_ballet)):
            if pasos[i]=='Relevé':
                cont_impar=1
                pasos_fraccion_impar.append(Fraction(cont_impar/len(posiciones_ballet)).limit_denominator())
                cont_impar=0
            else:
                pasos_fraccion_impar.append(0)
            if pasos[i]=='Retiré':
                cont_par=1
                pasos_fraccion_par.append(Fraction(cont_par/len(posiciones_ballet)).limit_denominator())
                cont_par=0
            else:
    ...
    df1= pd.DataFrame(pos,columns=['Fase 1 Posiciones'])
    df2=pd.DataFrame(pasos_fraccion_par,columns=['Retiré (Probabilidad)'])
    df3=pd.DataFrame(pasos_fraccion_impar,columns=['Relevé (Probabilidad)'])
    df4=pd.DataFrame(prob_pos,columns=['Probabilidad de salida'])
...
def run():
    ...
    label2=tk.Label(raiz,text='Combinaciones: '+str(comb),font=(50))
